...
export function SearchPageClient({ restaurants }: { restaurants: Restaurant[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return result
  }, [restaurants, query, sort, selectedCuisines, selectedPrices, selectedFeatures, selectedDietary])
      ...
      <div className="mt-5 flex gap-4 overflow-x-auto pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {CUISINE_CATEGORIES.map((cat) => {
          ...
          return (
            ...
            >
              <div className={`flex h-20 w-20 items-center justify-center rounded-2xl text-4xl shadow-sm ring-2 transition-all duration-200 group-hover:scale-105 group-hover:shadow-md ${active ? "bg-primary/10 ring-primary scale-105 shadow-md" : "bg-gradient-to-br " + cat.gradient + " ring-transparent"}`}>
                {cat.emoji}
              </div>
